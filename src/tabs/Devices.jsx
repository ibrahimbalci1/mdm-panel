...
export default function Devices({ devices, selectedDevice, setSelectedDevice, searchQuery, setSearchQuery, filterStatus, setFilterStatus, setCmdModal, setTab, exportCSV }) {
    ...
    <div style={{ display: "flex", gap: 18 }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <button className="btn pr" onClick={() => setTab("enrollment")}>➕ Yeni Cihaz</button>
        </div>
        ...
        <div style={{ border: "1px solid #1a1f35", borderRadius: "0 0 10px 10px", overflow: "hidden" }}>
          {filtered.length === 0 ? (
            ...
            <div key={d.id} className={`dr ${selectedDevice?.id === d.id ? "ac" : ""}`} onClick={() => setSelectedDevice(d)}>
              ...
              <div style={{ fontSize: 11, color: "#475569" }}>{d.last_seen_at ? (() => { const diff = Math.floor((Date.now() - new Date(d.last_seen_at)) / 1000); if (diff < 60) return `${diff}sn önce`; if (diff < 3600) return `${Math.floor(diff/60)}dk önce`; if (diff < 86400) return `${Math.floor(diff/3600)}sa önce`; return new Date(d.last_seen_at).toLocaleString("tr-TR"); })() : "—"}</div>
          ...
          <div className="card" style={{ overflow: "hidden", position: "sticky", top: 0 }}>
            ...
            <div style={{ padding: "12px 14px 14px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                {[{ c: "lock", l: "🔒 Kilitle" }, { c: "unlock", l: "🔓 Aç" }, { c: "reboot", l: "🔁 Başlat" }, { c: "locate", l: "📍 Konum" }, { c: "push_policy", l: "🛡️ Politika" }, { c: "disable_kiosk", l: "🔓 Kiosk Kapat" }, { c: "wipe", l: "🗑️ Sıfırla", d: true }].map(({ c, l, d }) => (
